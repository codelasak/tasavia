-- Corrected Database Schema Update for TASAVIA Dashboard
-- Run these commands in your Supabase SQL Editor

-- 1. First, add id columns and make them unique
DO $$
BEGIN
  -- Add id column to my_companies if it doesn't exist
  IF NOT EXISTS (
    ...
    ALTER TABLE my_companies ADD CONSTRAINT my_companies_id_unique UNIQUE (id);
  END IF;
END $$;

DO $$
BEGIN
  -- Add id column to companies if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE companies ADD CONSTRAINT companies_id_unique UNIQUE (id);
  END IF;
...
DO $$
BEGIN
  -- Add id column to pn_master_table if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE pn_master_table ADD CONSTRAINT pn_master_table_id_unique UNIQUE (id);
  END IF;
END $$;

DO $$
BEGIN
  -- Add id column to my_ship_via if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE my_ship_via ADD CONSTRAINT my_ship_via_id_unique UNIQUE (id);
  END IF;
END $$;

DO $$
BEGIN
  -- Add id column to purchase_orders if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
DO $$
BEGIN
  -- Add id column to po_items if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE po_items ADD CONSTRAINT po_items_id_unique UNIQUE (id);
  END IF;
...
DO $$
BEGIN
  -- Add id column to inventory if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 3. Create RLS policies (these will be created only if they don't exist)
DO $$
BEGIN
...
-- 5. Create update triggers for updated_at columns
DO $$
BEGIN
...
-- 6. Create inventory last_updated trigger
...
-- 8. Now safely update foreign key references to use new id columns
-- First drop existing foreign key constraints that might conflict
DO $$
BEGIN
  -- Drop and recreate purchase_orders foreign keys
  IF EXISTS (
  ...
  -- Drop and recreate po_items foreign keys
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
  ...
  -- Drop and recreate inventory foreign keys
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Now create the new foreign key constraints using the id columns
DO $$
BEGIN
  -- Create new foreign key constraints
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'purchase_orders_my_company_id_new_fkey'
  ) THEN
    ALTER TABLE purchase_orders ADD CONSTRAINT purchase_orders_my_company_id_new_fkey 
      FOREIGN KEY (my_company_id) REFERENCES my_companies(my_company_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'purchase_orders_vendor_company_id_new_fkey'
  ) THEN
    ALTER TABLE purchase_orders ADD CONSTRAINT purchase_orders_vendor_company_id_new_fkey 
      FOREIGN KEY (vendor_company_id) REFERENCES companies(company_id);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE purchase_orders ADD CONSTRAINT purchase_orders_ship_via_id_new_fkey 
      FOREIGN KEY (ship_via_id) REFERENCES my_ship_via(ship_via_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'po_items_po_id_new_fkey'
  ) THEN
    ALTER TABLE po_items ADD CONSTRAINT po_items_po_id_new_fkey 
      FOREIGN KEY (po_id) REFERENCES purchase_orders(po_id) ON DELETE CASCADE;
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'po_items_pn_id_new_fkey'
  ) THEN
    ALTER TABLE po_items ADD CONSTRAINT po_items_pn_id_new_fkey 
      FOREIGN KEY (pn_id) REFERENCES pn_master_table(pn_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'inventory_pn_id_new_fkey'
  ) THEN
    ALTER TABLE inventory ADD CONSTRAINT inventory_pn_id_new_fkey 
      FOREIGN KEY (pn_id) REFERENCES pn_master_table(pn_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'inventory_po_id_original_new_fkey'
  ) THEN
    ALTER TABLE inventory ADD CONSTRAINT inventory_po_id_original_new_fkey 
      FOREIGN KEY (po_id_original) REFERENCES purchase_orders(po_id);
  END IF;
END $$;

-- 9. Create PO number generation function and sequence
...
-- 10. Function to update PO totals
