...
interface PDFCompanyGridCompactProps {
  sections: CompanySection[]
  className?: string
}

function CompanySectionCompact({ section }: { section: CompanySection }) {
  const { title, company, shipToData, customContent } = section

  if (customContent) {
    return (
      <div>
        <h3 className="font-bold text-slate-900 mb-2 border-b pb-0.5 text-sm">{title}:</h3>
        <div className="text-xs">{customContent}</div>
      </div>
    )
  }
      ...
      <div>
        ...
        <div className="space-y-0.5 text-xs">
          ...
          {shipToData.address_details?.trim() && (
            <div className="whitespace-pre-line leading-tight">{shipToData.address_details}</div>
          )}
...
export default function PDFCompanyGridCompact({ sections, className = "" }: PDFCompanyGridCompactProps) {
  ...
  return (
    <div className={`grid compact-gap mb-4 compact-spacing ${gridCols} ${className}`}>
      {sections.map((section, index) => (
        <CompanySectionCompact key={index} section={section} />
      ))}
    </div>
  )
}
...
export function shouldShowShipToCompact(
