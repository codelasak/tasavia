...
export default function InvoicePDFPage({ params }: InvoicePDFPageProps) {
  const router = useRouter()
  const [salesOrder, setSalesOrder] = useState<SalesOrderPDFData | null>(null)
  ...
  const fetchSalesOrder = useCallback(async () => {
    try {
      ...
      if (soData && (!soData.invoice_number || soData.invoice_number.trim() === '')) {
        console.log('Generating new invoice number for sales order:', params.id)
